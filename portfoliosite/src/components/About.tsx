"use client";

import { motion } from "framer-motion";
import Image from "next/image";

const viewport = { once: true, margin: "-80px" } as const;

export default function About() {
  return (
    <section id="about" className="py-32 px-6 md:px-16">
      <div className="max-w-6xl mx-auto">

        {/* ── Bio row ── */}
        <div className="grid md:grid-cols-2 gap-16 items-center">
          {/* Text */}
          <div>
            <motion.h2
              initial={{ opacity: 0, y: 28 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={viewport}
              transition={{ duration: 0.7 }}
              className="font-display text-5xl md:text-6xl font-bold text-offwhite mb-10"
            >
              About
            </motion.h2>

            <motion.div
              initial={{ opacity: 0, y: 28 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={viewport}
              transition={{ duration: 0.7, delay: 0.1 }}
              className="space-y-5 text-offwhite/68 leading-relaxed text-[1.05rem]"
            >
              <p>
                I&apos;m Aahan — a product manager and computer science student
                at Northeastern University, originally from Bangalore, India.
              </p>
              <p>
                I build products by obsessing over users first. At CENTA, I
                owned features end-to-end for platforms serving 2 million
                teachers across 100 countries — from user research and PRDs
                through wireframes in Figma to shipping live pages. Before that,
                I built data pipelines at Graphene AI to surface customer
                insights for Fortune 500 clients like Costco.
              </p>
              <p>
                What sets me apart: I think in systems. My CS and economics
                background means I approach products through data, incentives,
                and user behavior — not just feature lists. I&apos;ve built ML
                models that predict crop yields for farmers, trained computer
                vision systems for healthcare, and run econometric analyses on
                real-world policy questions.
              </p>
              <p>
                Outside of work, I&apos;m a wildlife photographer with 300+ bird
                species documented, an award-winning filmmaker, and a WWF India
                conservation advocate. I believe the best PMs are deeply curious
                people — and I bring that curiosity to everything I touch.
              </p>
              <p>
                Currently seeking Product Management co-op opportunities for
                May–Dec 2026.
              </p>
            </motion.div>

            {/* Social links — larger & more visible */}
            <motion.div
              initial={{ opacity: 0, y: 28 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={viewport}
              transition={{ duration: 0.7, delay: 0.2 }}
              className="mt-9 flex gap-6 items-center"
            >
              <a
                href="https://github.com/aahan-rd"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="GitHub"
                className="text-offwhite/55 hover:text-copper transition-colors duration-200"
              >
                <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0112 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z" />
                </svg>
              </a>
              <a
                href="https://www.linkedin.com/in/aahan-desai-/"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="LinkedIn"
                className="text-offwhite/55 hover:text-copper transition-colors duration-200"
              >
                <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                </svg>
              </a>
              <a
                href="mailto:desai.aah@northeastern.edu"
                aria-label="Email"
                className="text-offwhite/55 hover:text-copper transition-colors duration-200"
              >
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                  <rect x="2" y="4" width="20" height="16" rx="2" />
                  <path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7" />
                </svg>
              </a>
            </motion.div>

            {/* Club tags — larger */}
            <motion.div
              initial={{ opacity: 0, y: 28 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={viewport}
              transition={{ duration: 0.7, delay: 0.28 }}
              className="mt-6 flex gap-3"
            >
              {["AeroNU", "NUHOC"].map((club) => (
                <span
                  key={club}
                  className="text-sm px-4 py-2 rounded-full border border-offwhite/20 text-offwhite/60 tracking-widest uppercase font-medium"
                >
                  {club}
                </span>
              ))}
            </motion.div>
          </div>

          {/* Headshot */}
          <motion.div
            initial={{ opacity: 0, y: 28 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={viewport}
            transition={{ duration: 0.7, delay: 0.15 }}
            className="relative"
          >
            <div className="relative aspect-[3/4] rounded-xl overflow-hidden">
              <Image
                src="/images/about/aahan-headshot.png"
                alt="Aahan Desai holding the AeroNU rocket"
                fill
                className="object-cover"
                style={{ objectPosition: "center 15%" }}
              />
              <div
                className="absolute inset-0"
                style={{
                  background:
                    "linear-gradient(to top, rgba(13,13,13,0.55) 0%, transparent 55%)",
                }}
              />
            </div>
            <div className="absolute -bottom-3 -right-3 w-full h-full rounded-xl border border-copper/20 -z-10" />
          </motion.div>
        </div>

      </div>
    </section>
  );
}
